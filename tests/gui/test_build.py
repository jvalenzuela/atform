...
class Build(unittest.TestCase):
    ...
    def test_submit_tests(self, mock_builder, _mock_dialog):
        ...
        # Extract calls to submit_test().
        submit_calls = [
            c for c in mock_builder.mock_calls if c[0].endswith(".submit_test")
        ]

        self.assertEqual(len(ids), len(submit_calls))
        for i in ids:
            name, args, _kwargs = submit_calls[i]
            self.assertTrue(name.endswith(".submit_test"))
    ...
    def test_callback(self, mock_builder, mock_dialog):
        ...
        cb = [
            args[0]
            for name, args, kwargs in mock_builder.mock_calls
            if name.endswith(".add_done_callback")
        ]
